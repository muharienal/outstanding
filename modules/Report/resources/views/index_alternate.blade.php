...
// Loop through reports and group status pekerjaan by unit and status
foreach ($reports as $report) {
    if ($report->show_status === 'Show') {
        $unit = $report->unit;
        $status = $report->status_pekerjaan;

        // Initialize the unit's array if not exists
        if (!isset($unitStatusData[$unit])) {
            $unitStatusData[$unit] = [];
        }

        // Count the status pekerjaan for the unit
        if (!isset($unitStatusData[$unit][$status])) {
            $unitStatusData[$unit][$status] = 1;
        } else {
            $unitStatusData[$unit][$status]++;
        }
    }
}
@endphp
...
<div class="container">
    <div class="row">
        ...
        <div class="card custom-card">
            <div class="card-body">
                <h4 class="card-title mb-3 card-title-custom">Progress Unit</h4>
                <div class="chart-container">
                    <canvas id="unitStatusChart" width="200" height="200"></canvas>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card custom-card">
            <div class="card-body">
                <h4 class="card-title mb-3 card-title-custom">Priority Status</h4>
                <div class="chart-container">
...
<script>
    $(document).ready(function() {
        ...
        // Function to get custom colors based on status pekerjaan
        function getStatusColor(status) {
            ...
            return colorMap[status] || getRandomColor();
        }

        // Function to create a grouped bar chart with rounded bars and customized options
        function createGroupedBarChart(canvasId, labels, data) {
            ...
            new Chart(ctx, {
                ...
                data: {
                    labels: labels,
                    datasets: data.map((unitData, index) => ({
                        label: unitData.label,
                        data: unitData.data,
                        backgroundColor: getStatusColor(unitData.label),
                ...
                options: {
                    ...
                    scales: {
                        ...
                        y: {
                            ...
                            beginAtZero: true,
                            ticks: {
                                stepSize: 1, // Set the y-axis scale interval to 1
                            },
                            grid: {
                    ...
                    plugins: {
                        legend: {
                            ...
                            labels: {
                                ...
                                boxWidth: 15, // Set the width of the legend square
        ...
        // Convert PHP-generated data to JavaScript format
        const unitStatusData = {!! json_encode($unitStatusData) !!};

        // Prepare data for the chart
        const statusLabels = ['Belum', 'IP', 'OK', 'Rutin']; // Define the desired order
        const unitLabels = Object.keys(unitStatusData);

        // Convert the unitStatusData object into an array for each status pekerjaan
        const statusChartData = statusLabels.map(status => {
            const unitCounts = unitLabels.map(unit => unitStatusData[unit][status] || 0);
            return {
                label: status,
                data: unitCounts,
            };
        ...
        // Create the grouped bar chart with rounded bars, custom colors, narrower width, legend at the bottom-left corner, square legend icons, padding between legend items, hidden y-axis labels, and hidden grid lines
        createGroupedBarChart('unitStatusChart', unitLabels, statusChartData);
    });
</script>
...
<script>
    ...
    function createDoughnutChart(canvasId, labels, data, backgroundColors) {
        ...
        new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                    legend: {
                        position: 'right', // Place the legend at the right side
                        labels: {
                            usePointStyle: true, // Display legend as square
                            boxWidth: 15, // Set the width of the legend square
                            padding: 15, // Add padding between legend items
                ...
                elements: {
                    arc: {
                        borderWidth: 4, // Set border width for the doughnut segments
                    },
                },
            },
        });
    ...
    // Convert PHP-generated data to JavaScript format
    const prioritasChartData = {!! json_encode($prioritasChartData) !!};

    // Define the desired order of legend labels
    const desiredPrioritasOrder = ['Emergency', 'High', 'Medium', 'Low'];

    // Rearrange the prioritasLabels array to match the desired order
    const prioritasLabels = desiredPrioritasOrder.filter(prioritas => prioritasChartData.hasOwnProperty(prioritas));
    const prioritasCounts = prioritasLabels.map(prioritas => prioritasChartData[prioritas]);
